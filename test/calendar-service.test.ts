...
describe('graph calendar service body conversion', () => {
  ...
  it('drops script content even with malformed closing tags', () => {
    expect(extractPlainTextFromGraphHtml('<script>alert(1)</script foo="bar"><p>Agenda</p>')).toBe('Agenda');
  });
  ...
  it('keeps common formatting readable as plain text', () => {
    expect(extractPlainTextFromGraphHtml('<p>Hello&nbsp;team</p><p>Line <strong>two</strong><br>Line three</p>')).toBe(
      'Hello team\n\nLine two\nLine three',
    );
  });
